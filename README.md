...
ページの閲覧回数を記録するChrome拡張機能を開発しています。<br/>
...
| 手順3 | 手順4 |
| :--- | :--- | 
| 拡張機能の画面を開いたら、右上の「デベロッパーモード」のトグルをオンにします。 | トグルをオンにすると、左に「パッケージ化されていない機能拡張を読み込む」というメニューが出てくるので、これをクリックします。ファイル選択画面が開くので、このプロジェクトの中にある「chrome-extension」のフォルダを選択します。|
...
必要なパッケージが存在しなければ、<br/>
chrome-extensionディレクトリにて、以下のコマンドを実行
...
現在、Google Chromeのローカルストレージにデータを保存しており、<br/>
定期的なデータの削除を行なっておらず、ローカルストレージを圧迫する仕様となっています。<br/>
